...
export default function AuditPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const conversionRate = (Number(formData.orders) / Number(formData.pageOpens) * 100).toFixed(1);
      const message = `You are GrowthOS AI, an expert restaurant growth analyst. Analyse this data and return ONLY a valid JSON object with no extra text, no markdown, no backticks.
...
CTR: ${formData.ctr}%
Page Opens: ${formData.pageOpens}
Orders: ${formData.orders}
Menu Conversion Rate: ${conversionRate}%
GMV: Rs ${formData.gmv}
ATV: Rs ${formData.atv}
Cancellation Rate: ${formData.cancellationRate}%
Rating: ${formData.rating}
New Users: ${formData.newUserPercent}%
Returning Users: ${formData.returningUserPercent}%

Return this exact JSON structure:
{"overallScore":75,"summary":"2 sentence summary","leaks":[{"severity":"critical","metric":"metric name","current":"current value","benchmark":"what it should be","impact":"revenue impact estimate","problem":"what is wrong","cause":"why it is happening","fix":"exact action to take"}],"topWins":["quick win 1","quick win 2","quick win 3"],"weeklyPriority":"single most important action this week"}`;
